...
Companions can now be hurt and can die. Rather than independent enemy-AI targeting (a deliberate scope choice), a companion trades blows with whatever it's currently fighting: every hit it lands, the enemy swings back for a fraction (`COMPANION_RETALIATION_PCT`, 45%) of its own attack damage. Companions have real HP (100, tracked and broadcast for real in `CompanionSnapshot` instead of the old hardcoded-full value) and can be worn down and killed in a prolonged fight; a dead companion goes still, is hidden from the world, and gets back up on its own after `COMPANION_REVIVE_MS` (25s), full health, at its owner's side — no dismiss/re-recruit needed. Verified live over the wire: recruiting Bran Fieldhand and walking him into a Moonlit Wolf fight showed his tracked HP dropping from retaliation damage in real snapshots, exactly as designed.
...
7. **Full independent enemy-vs-companion AI targeting** — companions currently take damage via retaliation on their own attacks (a deliberate first-pass scope choice, see "Companions" above), not by being independently picked as targets by enemy AI the way players are.
