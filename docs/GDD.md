...
Four NPCs — Bran Fieldhand (Threadhold), Thorn Ash-Debt (Ashmire), Solace Stillwater (The Frayedge), and Nix Fray (The Frayedge) — have a `recruits: true` option on their signature choice. Choosing it sets `CharacterState.companionId` and the Room spawns a real, simulated companion entity, not just a flag: it follows you (closing distance once you're more than a few units away), crosses zones with you instantly when you travel, and fights — it picks the nearest live enemy within its aggro radius, closes in, and attacks on a cooldown, with kill credit (XP/loot) going to you. The HUD shows "Traveling with `<name>`" while one is active. One companion at a time; there's no dismiss/swap UI yet, and the recruiting NPC themselves stays put at their original location as a separate, talkable entity — the companion is a second, following copy, not a relocation. Companions can't be damaged in this pass (a deliberate scope cut — no death/respawn handling for them yet).

Thirty-three NPCs are fully wired end-to-end today, each with a **signature choice** — a real decision with real faction-loyalty consequences — demonstrating the pattern the rest of the ~60-character planned roster (see below) is designed against. Four of them (marked below) can be recruited as a companion instead of just answering a faction-flavored choice:
...
Being explicit about scope, because it matters: the original narrative design for this game specifies a roster of **60+ named recurring characters** across seven planned regions (Threadhold, Ashmire, Sunken Llyr, Mourncrown, Spirechain, Frayedge, and the endgame Moonthread itself), a full NPC relationship web (rivalries, alliances, death cascades), an 8-chapter story, and a much richer ending system with secret endings and NPC-survival variants. All six non-endgame regions are real, working zones today, and 33 characters are wired end-to-end in them, including a first working slice of the relationship web (NPCs whose dialogue reacts to choices made with *other* NPCs) and a first working slice of joinable companions (four NPCs who follow and fight alongside you — see "Playable conversations" and "Companions" above). The remaining ~27 are designed in the narrative bible — dialogue voice, faction ties, signature choices, and relationship webs are written — but not yet wired into the game. Still unbuilt: rivalry/alliance pairs as a first-class structure (rather than hand-picked cross-references); death cascades where one NPC's fate removes or transforms another; a dismiss/swap-companion UI and more than one active companion at a time; and companion combat depth (they can't be damaged or die yet, and fight with one flat attack, not their own kit). The Moonthread itself remains the one region from the brief with no zone at all yet — reaching it is meant to be a late-game/endgame beat, not an early travel-point destination. Building the rest out is authoring content and relationship-graph data against the systems already in place, not new core engineering.
...
1. **The rest of the NPC roster** — the remaining ~27 designed characters across the six built zones, plus the Moonthread endgame zone itself (not yet built — reaching Selen is meant to be a late-game destination, not another Threadhold travel point). Also unbuilt: rivalry/alliance pairs as first-class relationship-graph data, death cascades (one NPC's fate removing or transforming another), a dismiss/swap-companion UI and multiple simultaneous companions, and companion combat depth (damageable, own ability kits, not one flat attack). Content + data authoring against the existing dialogue/memory/faction/cross-reference/companion system, not new core engineering.
